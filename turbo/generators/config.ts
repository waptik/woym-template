...
export default function generator(plop: PlopTypes.NodePlopAPI): void {
	plop.setGenerator("init", {
		description: "Generate a new package for the Woym Monorepo",
		prompts: [
			{
				type: "input",
				name: "name",
				message: "What is the name of the package? (You can skip the `@woym/` prefix)",
			},
			{
				type: "input",
				name: "deps",
		...
		actions: [
			(answers) => {
				if ("name" in answers && typeof answers.name === "string") {
					if (answers.name.startsWith("@woym/")) {
						answers.name = answers.name.replace("@woym/", "")
					}
				}
			...
			async (answers) => {
				...
				if ("name" in answers && typeof answers.name === "string") {
					...
					execSync(
						`pnpm biome check --write packages/${answers.name}/**`,
